...
startTable:    
		// graycode conversion lookup table
...
getInput:
	...
	lsr input // fix off by one bit issue
	lpm original, z                          ;Load the first instance of the test data from Program Memory
	add input, r26
    cp zl, input                           ; have we made it to input index
    brne getInput                                            ; if not, repeat with next byte...
    movw z,x
...
//TEMP
	...
	rjmp getInput // continuously grab input
	//rjmp decideStep
	rjmp display
